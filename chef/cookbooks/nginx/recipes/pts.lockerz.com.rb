...
template "/etc/nginx/sites-available/pts.lockerz.com" do
    mode "0755"
    owner "root"
    group "root"
    source "pts.lockerz.com/vhost.erb"
    notifies :reload , resources(:service => "nginx")
    variables({
        :ssl => false,
        :envName => envName,
...
link "/etc/nginx/sites-enabled/pts.lockerz.com" do
	to "/etc/nginx/sites-available/pts.lockerz.com"
end
...
lm_watcher do
	...
	logName "pts_add2any"
	logDisplay "Add2Any Pixel Tracking Service"
	logFilename "%s/pts/access.log" % [fsLogRoot]
	rotationInterval 15
end
end
