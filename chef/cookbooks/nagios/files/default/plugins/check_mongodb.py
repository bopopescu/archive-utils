...
def main(argv):
    ...
    elif action == "slave_sync":
		check_slave_sync(host,port,warning,critical)
...
def check_rep_lag(host, port, warning, critical):
    try:
        ...
        #res = con.admin.command( "printSlaveReplicationInfo","1" )
        #res = con.admin.command(pymongo.son_manipulator.SON([('printSlaveReplicationInfo', 1)]))
        #lag = int(call( "mongo %s --eval 'db.printSlaveReplicationInfo()'|grep '='|awk '{print $2}'" % host, shell=True ))
        lag = int(os.popen( "mongo %s --eval 'db.printSlaveReplicationInfo()'|grep '='|awk '{print $2}'" % host ).read())
		#res = sys
        #print "Lag: [%s]" % lag
        #sys.exit(2)
        ...
        if lag >= critical:
            print "CRITICAL - Max replication lag: %i" % lag
            sys.exit(2)
        elif lag >= warning:
            print "WARNING - Max replication lag: %i" % lag
            sys.exit(1)
        else:
            print "OK - Max replication lag: %i" % lag
            sys.exit(0)
